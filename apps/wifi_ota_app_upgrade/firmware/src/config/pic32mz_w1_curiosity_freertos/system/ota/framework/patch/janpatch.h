/*Code is inspired by : https://github.com/janjongboom/janpatch*/
// DOM-IGNORE-BEGIN
/*******************************************************************************
...
#ifdef JANPATCH_DEBUG
#define JANPATCH_DEBUG      SYS_CONSOLE_PRINT
#else
#define JANPATCH_DEBUG(...)  while(0)
#endif


#ifndef JANPATCH_ERROR
#define JANPATCH_ERROR      SYS_CONSOLE_PRINT
#endif
#define JANPATCH_STREAM     SYS_FS_HANDLE
...
typedef struct {
    ...
    JANPATCH_STREAM  stream;// Modified : Removed pointer symbol
    int32_t         position;
} janpatch_buffer;
...
typedef struct {
    ...
    // function signatures
    size_t (*fread)(JANPATCH_STREAM,void*, size_t);
    size_t (*fwrite)(JANPATCH_STREAM,const void*, size_t);
    int    (*fseek)(JANPATCH_STREAM, int32_t, SYS_FS_FILE_SEEK_CONTROL);
    int32_t   (*ftell)(JANPATCH_STREAM);
...
static size_t jp_fread(janpatch_ctx *ctx, void *ptr, size_t size, size_t count, janpatch_buffer *buffer) {
    ctx->fseek((JANPATCH_STREAM)buffer->stream, buffer->position, SYS_FS_SEEK_SET);
    size_t bytes_read = ctx->fread((JANPATCH_STREAM)buffer->stream,ptr, count);
    if(bytes_read == -1)
    {
        /* There was an error while reading the file. Close the file
                 * and error out. */
        SYS_FS_FileClose(buffer->stream);
...
static size_t jp_fwrite(janpatch_ctx *ctx, const void *ptr, size_t size, size_t count, janpatch_buffer *buffer) {
    ...
    ctx->fseek((JANPATCH_STREAM)buffer->stream, buffer->position, SYS_FS_SEEK_SET);
    size_t bytes_written = ctx->fwrite((JANPATCH_STREAM)buffer->stream,ptr, count);
    SYS_FS_FileSync((JANPATCH_STREAM)buffer->stream);
