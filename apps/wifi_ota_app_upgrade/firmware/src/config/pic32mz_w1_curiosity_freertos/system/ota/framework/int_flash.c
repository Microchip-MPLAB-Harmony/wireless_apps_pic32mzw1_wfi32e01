...
OSAL_MUTEX_HANDLE_TYPE mutex_nvm_g;
bool mutex_nvm_g_status;
#endif
//---------------------------------------------------------------------------

#if (OTA_NVM_INT_CALLBACK_ENABLE == true)
...
void INT_Flash_Initialize(void)
{
        ...
        if(OSAL_MUTEX_Create(&mutex_nvm_g) == OSAL_RESULT_FALSE)
        {
            mutex_nvm_g_status = false;
        }
        else
        {
            mutex_nvm_g_status = true;
        }
    #endif
}
//---------------------------------------------------------------------------
...
bool INT_Flash_Write(uint32_t addr, uint8_t* buf, uint32_t len)
{
    ...
    if (OSAL_MUTEX_Lock(&mutex_nvm_g, OSAL_WAIT_FOREVER) == OSAL_RESULT_TRUE)
    {
        while (len)
        {
            while( NVM_IsBusy() ) ;
