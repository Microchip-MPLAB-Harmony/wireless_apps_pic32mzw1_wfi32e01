...
{"0": {
    ...
    "content": "| Command | Description | . | wlan set config &lt;ssid&gt; &lt;ssid_length&gt; &lt;channel&gt; &lt;open/wpa2/wpam/wpa3/wep&gt; &lt;password&gt; | Configures the SSID, SSID length, channel number and the security of the AP that DUT wants to connect. NOTE: Valid channel numbers are in range 1-13 or 0 to scan all channels wlan connect Connects the device to the AP that is configured in the command “wlan set” | . | wlan connect | Connects the device to the AP that is configured in the command “wlan set” | . | wlan scan &lt;active / passive&gt; &lt;channel&gt; &lt;scan time in ms&gt; | Rus either a active or passive (as per the input) scan on the channel number specified for the time issued in ms. | . | wlan rssi | Gets the RSSI of current association | . | wlan config save | Saves the parameters issued in command “wlan set” to flash memory. On restart an attempt is made to establish a connection to the credentials saved in flash memory. | . | wlan set regdomain | Sets the regulatory domain | . | wlan get regdomain &lt;all / current&gt; | Displays the regulatory domain all – request all regulatory domains; current - request current regulatory domain | . |   |   | . ",
    ...
    "doc": "Wi-Fi Soft AP (driver mode)",
    "title": "Supported Commands",
    "content": "| Command | Description | . | wlan set config &lt;ssid&gt; &lt;ssid_length&gt; &lt;ssid_visible&gt; &lt;channel&gt; &lt;open/wpa2/wpam/wpa3/wpa3m/wep&gt; &lt;password&gt; | Configures the SSID, SSID length, channel number and the security of the AP that DUT wants to connect. NOTE: Valid channel numbers are in range 1-13 or 0 to scan all channels wlan connect Connects the device to the AP that is configured in the command “wlan set” | . | wlan ap start | Start the AP | . | wlan ap stop | Stop the AP | . | wlan show devices | The connected devices are shown – MAC and IP addresses are displayed | . | wlan config save | Saves the parameters issued in command “wlan set” to flash memory. On restart DUT is started as AP with the credentials stored in flash. | . | wlan set regdomain | Sets the regulatory domain | . | wlan get regdomain &lt;all / current&gt; | Displays the regulatory domain all – request all regulatory domains; current - request current regulatory domain | . |   |   | . ",
    ...
    "content": "This example application acts as a Wi-Fi Station(STA) to connect to Access point(AP) and perform OTA application update process to download an image present in the HTTP server. ",
    ...
    "content": "Over the Air (OTA) firmware upgrade feature is designed with a two step process, Image Downloading and Image Programming process. Image Downloading process is now processed by the main application firmware. Abstraction model: . | Application is responsible to start the OTA process by calling OTA_start() function to download the new image into the Image Store in the external flash (sst26vf). | Image downloading is done through HTTP protocol. | Bootloader is responsible for the programming process, which loads and copies the best image from Image Store to Program area and jumps to the Application. The bootloader also supports ‘Fail-Safe’ boot scheme which the firmware automatically rolls back if the image has not been run correctly at the previous boot. | Image-Store in external flash (sst26vf) can have multiple images, including the default image for factory reset. | . Application Image Structure : . OTA application image structure is organised as shown in below picture. Total application length includes header bytes as well. Bootloader copies Application Image Length bytes from Image-Store to Program-flash area. Application Header Structure : . Every ota image includes a header to keep track of image status as shown in below table: . OTA application and bootloader maintains IMAGE_HEADER_SIGNATURE byte, which is of one byte, located at 4096th byte (the last byte on the sector, refer Application Image Structure above) on the image header. The value should contain the same with the status field on the top. This is to ensure sector image integrity. If values are different, it is suspected the power cut or unknown reboot happened during the sector programming and image in that particular slot is treated as invalid image. | order - it will be incremented for every image downloaded using OTA. If this value reaches 15 and then OTA process downloads a new image , value will 1. | slot - it will indicate the slot in the external flash where the image is located. | 0 for defual image | 1 for slot 1 | 2 for slot 2 | . | . OTA application follows below steps while downloading an image from HTTP server: . | Allocating slot: When OTA process is initiated, the OTA framework will try to allocate a slot for the new image in the external flash. This slot will either be a blank slot, the lowest ranked or invalidated application firmware image slot. The External flash will be configured for 3 slots for, storing upto 3 OTA images. Slot 0 is reserved for the factory image and can not be replaced with an image obtained through OTA process. Only slot 1 and slot 2 will be used for storing new OTA images. Application allocates the slots in sequential order, meaning ota image downloaded during first OTA process will be stored in slot1 and 2nd OTA image downloaded during 2nd OTA process will be stored in the slot2. If the highest slot (slot 2) is already filled with the latest OTA image and user initiates OTA in this situation again, then the slot1 will be allocated for the new OTA image. | The transport layer (HTTP protocol) starts downloading the image. | OTA framework verifies the image by checking the signature field once transport layer finished receiving all data. Signature is a SHA-256 hash. If the downloaded image is valid, the OTA framework updates “Boot-Control”-header, status field of the current application to “Disabled” 0xF0. | The application will be notified about OTA process completion using user registered call-back function. If OTA download has successfully finished, the application must reset the system to start the programming process. | . Flow Chart Of The Application: . ",
    ...
    "doc": "Over The Air (OTA) Upgrade Using Wi-Fi",
    "title": "Running the Application",
    "content": ". | Open the project “wifi_ota_app_upgrade”. | Demo is configured with the default wifi credentials shown in the image below. To change the configurations launch the Harmony 3 configurator and update the home AP credentials for STA Mode and generate code. | As a part of OTA process device will try to connect to user defined HTTP server. If device is able to connect to server without any error, it will try to download the image “.ota” (image name should be defined correctly by user). | User can use any HTTP server. | User may also use python command to create a local http server using below steps: . | Open command prompt and change driectory to the folder where ota image is present. | Use below python command in command prompt: . python -m http.server 8000 . | . | . | Configure HTTP server address and ota image name in “app.h” file using macro “#define SYS_OTA_URL”. | User need to disable “Add linker file” option to exclude MHC generated default linker file, and add the required custom linker file present in path ..\\wifi_ota_app_upgrade\\firmware\\\\src\\ota . | Generate the code using MHC. | It is required to integrate the bootloader and ota application image and create a single unified HEX file. To integrate 2 images we can use hexmate tool, which is readily available with MPLABX package as part of the standard installation. To combine the hex files - . | User should load the “ota_bootloader” project located in the “wireless” apps folder of this repo and include it into “wifi_ota_app_upgrade” project as a “Loadable” component. For this, right click on the “wifi_ota_app_upgrade” project, click on “properties” and select “ota_bootloader” project. User need to make sure that the steps mentioned in “ota_bootloader” reference document is followed, before this step. | Click on “Apply” button to make the applied changes effective: . | . | It is required to perform a “post-build” step to create ota image with file extension “.ota” (which can be placed in the server and downloaded during OTA process) and factory reset image. During “post-build” a defined header will be included to the image using python script. | All required files for post-build process are present in “utilities” folder which is present in the path apps/wifi_ota_app_upgrade/. | Right click on the “wifi_ota_app_upgrade” project and click on properties. | Select “building”, insert below command and click “OK”: . python ../../utilities/pic32mzw1/ota/post_process.py -d \${Device} -p \${ProjectDir} -c \${ConfName} -i \${ImagePath} \${ImageDir} \${ImageName} \${IsDebug} . Note: python should be present in the system variable path. | User can download any valid binary or hex image. ( Not necessary to be image with “.ota” extension ) | . | Build and program the application. | Place the .ota image into the HTTP server. The .ota image can be found in below path which is generated during “wifi_ota_app_upgrade” project build : ..\\firmware\\wifi_ota_app_upgrade.X\\\\dist\\\\pic32mz_w1_curiosity_freertos\\\\production . | Connect to a USB to UART converter to UART1 and Open a Terminal application (Ex.:Tera term) on the computer. Configure the serial settings as follows: . | Baud : 230400 | Data : 8 Bits | Parity : None | Stop : 1 Bit | Flow Control : None | . | The device will connect to the Home AP and print the IP address obtained. | From the DUT(Device Under Test), user can ping the Gateway IP address. | Once IP address is obtained the device will initiate OTA process. | User should make sure that both HTTP server and the PIC32MZW1 device are part of same wifi network (or connected to same Home AP). | Once image is downloaded successfully, the application will print a message in the console. User need to reset the device to load the new image. | If OTA upgrade fails, user need to reset the device to initiate OTA process again. | During reset, device will check if any newly downloaded image is available in the external flash(sst26vf): . | if yes, bootloader will program new image to program-flash area of the device from external flash. | if programming is successful bootloader will hand over control and application will start executing. | . | if no new image is available then bootloader will hand over control without programming any image and application image already present in the program-flash area will start executing | . | If any new image is available in external flash bootloader will try to program the new image. During programming if bootloader finds that the image is not valid (based on certain criteria, check “ota_bootloader” project reference for more details) then , bootloader will stop programming that particular image and check for other highest ranked (check “ota_bootloader” project reference for more details) valid image prepsent in the external flash and start programming it. Once programming is completed successfully , bootloader will hand over control and application will start executing. | User will come to know if new image is running by checking the slot number in console print ( as mention above under “Application Header Structure”- topic ) | . ",
    ...
    "doc": "WiFi Touch Demo",
    "title": "Setting up PIC32MZ W1 Curiosity Board",
    "content": ". | Connect a programmer tools such as SNAP, ICD4, etc to ICSP header. | On the GPIO Header (J207), connect U1RX (PIN 13) and U1TX (PIN 23) to TX and RX pin of any USB to UART converter | Laptop/ Mobile (to run the TCP Client on it) | Jumper J211 is connected between pins 2-3 (closer to xpro header) | Jumper J209 is disconnected | Jumpers J301 are disconnected (to use external programmer tool) | QT7 Xpro board is connected to Curiosity Xpro header | . ",
    ...
    "doc": "WiFi Touch Demo",
    "title": "Running the Application",
    "content": ". | Open the project and launch Harmony3 configurator. | Currently Net Service is configured to run as TCP Server running on port 5555. In case the user wants to change this config, please make the changes in the Net Service Module as shown below: | . | Configure the Wi-Fi Service STA mode as per your target AP parameters: | . | Save configurations and generate code via MHC | Build and program the generated code into the hardware using its IDE | Open the Terminal application (Ex.:Tera term) on the computer | Connect to the “USB to UART” COM port and configure the serial settings as follows: . | Baud : 115200 | Data : 8 Bits | Parity : None | Stop : 1 Bit | Flow Control : None | . | The device shall come up as STA and then as per the Net Service configuration, the TCP Server shall come up, awaiting a connection from a TCP Client. | Connect a Laptop to the same AP and run the provided python script. (tested with Python v3.8 and Windows 10) | Connect the python client giving the server IP as the IP of the Board, and the port as 5555. All operations from the QT7 Xpro board will now be visible real-time on the GUI: | . ",
    ...
    "doc": "WiFi Touch Demo",
    "title": "Known Issues:",
    "content": "While regenerating the code using MHC, make sure not to overwrite following two functions in touch.c. These are generating as qtm_ptc_ while they should be qtm_cvd_ causing build issues. Simply changing the function to qtm_cvd_ resolves the issue. ",
    ...
    "doc": "OTA Boot Loader",
    "title": "OTA Bootloader framework Architecture",
    "content": ". | Abstraction Model: . | OTA application is responsible to start the OTA process and download new image to Image Store in the external flash (sst26vf). | Image downloading is done through HTTP protocol. | Bootloader is responsible for the programming process, which loads and copies the best image from Image Store to Program area and jumps to the Application. The bootloader also supports ‘Fail-Safe’ boot scheme which the firmware automatically rolls back if the image has not been run correctly at the previous boot. | Image-Store can have multiple images, including the default image for factory reset. | . | During each system boot-up, bootloader checks if it needs to program any image from the external flash. Bootloader goes to program mode, if- . a) any newly downloaded image present in the external flash, . b) if the already present image in the program flash is identified as “not safe” during previous boot. There are two conditions : . | whether the Application Image in Program-Flash area is valid (indicated by the STATUS field, value of 0xF8 in image Header), and | whether it has been confirmed that no errors were present during the previous boot (indicated by the STATUS field (of image Header) value of 0xF8 in the original image located in the external flash). | . According to bootloader logic if these two conditions are satisfied it will not go to “Program Mode” and the bootloader immediately jumps to the application image present in the program-flash area of the device. | If two conditions mentioned in the step 2 are not satisfied, the Bootloader switches to Image Program Mode. In Image Program Mode bootloader follows Image Programming sequence, which finds the highest ranked image in Image-Store(external flash), erases the Program-Flash area and copies the selected image to the Program-Flash area if the image is successfully verified. As the newly downloaded image is set as the highest rank, during the first boot time after the Image-downloading, the Bootloader attempts to load the newly downloaded image at the first try. Bootloader choses the highest ranked image to boot. The images are ranked in following order: . | The latest downloaded Image . | The next booted image . | The next known booted image . | Default (Golden) image . | . | If the image is not valid, the bootloader invalidates the image by setting “Invalidate” 0xF0 in STATUS field of header present in the external flash and restarts the Image-Programming sequence. | If image is verfied successfully, Bootloader updates STATUS field of the image header in external flash- . a) checks the header-status field, if it is “Downloaded” (0xFE) updates it as “Unbooted”(0xFC) and jumps to application image. b) if already a validated image (header-status value is “0xF8”), it starts running application image ignoring above mentioned step a). | Header structure : . | Flow chart : . | . ",
    "url": "http://localhost:4000/wireless_apps_pic32mzw1_wfi32e01/apps/ota_bootloader/readme.html#ota-bootloader-framework-architecture",
    "relUrl": "/apps/ota_bootloader/readme.html#ota-bootloader-framework-architecture"
  },"100": {
    "doc": "OTA Boot Loader",
...
var home_index_string = "Wireless apps for PIC32MZW1";
